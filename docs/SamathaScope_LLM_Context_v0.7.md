...
---
# SamathaScope - LLM Context Pack (v0.7)
...
Sky Tower in v0.7:
- each dropped stone is built from `2-4` seeded convex lobes
- landings use a custom stack simulator with swept support checks rather than general rigid-body dynamics
- the stack behaves like a springy, wobble-prone tower instead of a rigid slab pile
- artefact score directly increases wobble, jitter, and recovery time
- support comes from frictional resistance and slip damping, not glue or weld joints
- invisible side walls keep stones inside the playfield instead of letting them escape into the HUD
- fall timing is intentionally faster than earlier builds
...
- `Sky Tower`
  - one tap releases one hovering composite stone
  - the next stone appears only after the released one settles or fails
  - artefact score drives wobble amplitude, jitter, and slower recovery
